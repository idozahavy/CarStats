...
- On desktop platforms (Windows / Linux / macOS), `file_picker` does not write the bytes passed to `saveFile`. `ExportService` compensates by writing the file manually after the path is chosen. See [export_service.dart:36](lib/services/export_service.dart#L36).
