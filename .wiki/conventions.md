...
- Global state lives in `ChangeNotifier`s:
  - `ThemeProvider`, `SettingsProvider`, `LocaleProvider` (persisted via `SharedPreferences`)
...
- All user-facing strings live in [lib/l10n/app_en.arb](lib/l10n/app_en.arb) (template) and [lib/l10n/app_he.arb](lib/l10n/app_he.arb).
- Code reads them via `AppLocalizations.of(context)!` (typically aliased as `final l = AppLocalizations.of(context)!;` at the top of `build`).
- Generated files (`lib/l10n/app_localizations*.dart`) are regenerated by `flutter pub get` / `flutter gen-l10n`. Do not hand-edit.
- Do not translate units (`km/h`, `g`, `m/s`, `°`, `hPa`); only the labels around them.
- For non-widget code that needs a localised string, capture it from the nearest `BuildContext` before any `await`.
